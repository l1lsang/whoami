...
export function ProjectVisual({ project, className = '' }: ProjectVisualProps) {
  ...
  if (project.image) {
    return (
      <div
        className={`overflow-hidden border border-ink/20 bg-surface ${className}`}
      >
        <img
          src={project.image}
  ...
  if (project.slug === 'dream-defenders') {
    return (
      <div
        className={`relative overflow-hidden border border-ink/20 bg-[#20231f] ${className}`}
        role="img"
        aria-label="드림 디펜더스 Unity 게임 화면 플레이스홀더"
      >
        ...
        <div className="absolute inset-x-0 top-0 flex h-10 items-center justify-between border-b border-white/20 bg-black/20 px-4 font-mono text-[9px] text-white/60">
          <span className="flex items-center gap-2">
            <Gamepad2 size={12} className="text-unity" /> GAME VIEW
          </span>
          <span>STAGE 03</span>
        </div>
        ...
        <div className="absolute bottom-[16%] right-[12%] flex gap-2">
          {[1, 2, 3].map((enemy) => (
            <span
              key={enemy}
              className="block h-10 w-6 border border-unity/70 bg-unity/20"
            />
          ))}
        </div>
        ...
        <span className="absolute right-4 top-14 font-mono text-[9px] text-white/60">
          WAVE 4 / 6
        </span>
      </div>
    )
  ...
  return (
    <div
      className={`relative overflow-hidden border border-ink/20 bg-[#20231f] p-4 ${className}`}
      role="img"
      aria-label="디스코드 경제 게임봇 채팅 화면 플레이스홀더"
    >
      <div className="absolute inset-y-0 left-0 w-12 border-r border-white/20 bg-black/20 p-2">
        <div className="grid size-8 place-items-center bg-unity/20 text-unity">
          <Bot size={16} />
        </div>
      ...
      <div className="ml-12 flex h-full flex-col border border-white/20 bg-[#292d28]">
        <div className="flex h-10 items-center justify-between border-b border-white/20 px-3 font-mono text-[9px] text-white/60">
          <span># 경제-게임</span>
          <span className="flex items-center gap-2 text-unity">
            <CircleDollarSign size={12} /> 24,850 DC
          </span>
        </div>
